...
const collectionPokemon: Collection<PokemonData> = client
  .db("wpl")
  .collection<PokemonData>("pokemon");
...
async function fetchEvolutionChain(
  speciesUrl: string
): Promise<{ id: string; name: string; sprite: string }[]> {
  try {
    ...
    const traverseChain = async (chainLink: any) => {
      ...
      if (chainLink.evolves_to.length > 0) {
        await Promise.all(
          chainLink.evolves_to.map(
            async (evolution: any) => await traverseChain(evolution)
          )
        );
      }
    };
...
async function loadPokemonsFromApi(collectionPokemon: Collection<PokemonData>) {
  ...
  try {
    ...
    if (count > 0) {
      console.log(
        "Pokémon data already exists in the database. Skipping fetch operation."
      );
      return;
    }
...
async function getCaughtPokemon(userId: any) {
  try {
    ...
    const caughtPokemonData = await collectionPokemon
      .find({ id: { $in: caughtPokemonIds } })
      .toArray();
...
async function filteredPokemon(searchTerm: string): Promise<PokemonData[]> {
  ...
  try {
    const searchResult = await collectionPokemon
      .find({
...
async function updateSelectedPokemon(
  userId: string,
  selectedPokemonId: string
): Promise<void> {
  try {
    ...
    console.log(
      `Selected Pokémon updated successfully for user with ID ${userId}`
    );
  } catch (error) {
...
async function getSelectedPokemon(
  ...
  try {
    return (await collectionPokemon.findOne({ id: selectedPokemonId })) || null;
  } catch (error) {
    throw new Error(
      `An error occurred while fetching selected Pokémon: ${error}`
    );
  }
}
